...
public class FlappyBird extends JPanel implements ActionListener, KeyListener, MouseListener{
    ...
    public void placePipes(){
        //(0-1) * pipeHeight/2 ->(random no between 0-256)
        //128
    ...
    public void move(){
        ...
        for(int i=0; i<pipes.size(); i++){
            ...
            if (!pipe.passed && bird.x > pipe.x + pipe.width) {
                pipe.passed = true;
                score += 0.5; //because there are 2 pipes 0.5 score for each pipe
            }
    ...
    public void keyPressed(KeyEvent e) {
        if(e.getKeyCode() == KeyEvent.VK_SPACE){
            velocityY = -9;
            if (gameOver) {
                //restart the game by resetting the original declaration
